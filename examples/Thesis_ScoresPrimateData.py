...
for sp in splits:
    print(sp)
    F = tree.flattening(sp, DTable)
    SF = tree.subFlatteningAltFast(F)
    SF_2 = tree.subFlatteningAltFast(F, S=scaledHMatrix(0.5)[0])
    SF_3 = tree.subFlatteningAltFast(F, S=scaledHMatrix(3)[0])
    SF_10 = tree.subFlatteningAltFast(F, S=scaledHMatrix(15)[0])
...
fig.tight_layout()
fig.savefig('Scores_primate.pdf')
plt.show()
