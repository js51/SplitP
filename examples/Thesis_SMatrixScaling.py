...
with sns.color_palette("colorblind"):
    ...
    plt.errorbar(matNames, list(finalResultsTable[0]), yerr = list(finalResultsTable[1]), linewidth=0.75, markersize=2, marker='o', capsize=2.5, elinewidth=0.5)
    plt.xlabel("$\lambda$ for scaled matrix $S=H^{(\lambda)}$")
    plt.ylabel("Measure of performance 1")
    plt.tight_layout()
    plt.savefig('MOP1_6TAX.pdf')
    plt.show()

    plt.figure(figsize=(4,3))
    plt.errorbar(matNames, list(finalResultsTable[2]), yerr = list(finalResultsTable[3]), linewidth=0.75, markersize=2, marker='o', capsize=2.5, elinewidth=0.5)
    plt.xlabel("$\lambda$ for scaled matrix $S=H^{(\lambda)}$")
    plt.ylabel("Measure of performance 2")
    plt.tight_layout()
    plt.savefig('MOP2_6TAX.pdf')
    plt.show()
